...
export class StagehandClient {
  ...
  async startJobSearch(params: JobSearchParams, platform: string) {
    ...
    await page.act(`Search for ${params.keywords.join(' ')} jobs in ${params.location}`);
    ...
    if (params.experienceLevel.length > 0) {
      await page.act(`Filter by experience level: ${params.experienceLevel.join(', ')}`);
    }
  ...
  async extractJobs(searchSessionId: string): Promise<Job[]> {
    ...
    try {
      ...
      return extractedData.jobs.map((job) => ({
        id: `${searchSessionId}_${Date.now()}_${Math.random().toString(36).substring(7)}`,
        title: job.title,
        company: job.company,
        ...
        url: job.url,
        platform: new URL(page.url()).hostname.replace('www.', '').split('.')[0],
